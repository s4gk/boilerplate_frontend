...
const roleIcons: Record<string, ReactNode> = {
  super_admin: <ShieldCheck className="size-4 shrink-0 text-foreground" />,
  administrativo: <FileEdit className="size-4 shrink-0 text-foreground" />,
  caja_ventas: <Eye className="size-4 shrink-0 text-foreground" />,
}
...
function getRoleIcon(roleId?: string) {
  ...
  return roleIcons[normalized] ?? <User className="size-4 shrink-0 text-muted-foreground" />
} 
                  ...
                  <TableRow key={employee.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5 text-sm text-foreground">
                        {firstRole ? (
                          <>
                            {getRoleIcon(firstRole.id)}
                            <span>{firstRole.name}</span>
                          </>
                        ) : (
                          <span>Sin rol</span>
                        )}
